			...
			for(int is1=0;is1<20;is1++)
				...
				for(int i2=0;i2<ed.PMTWF->at(i1).size();i2++)
					...
					else
					{
						...
						{
							hsum[is1]+=(-1.*(ed.PMTWF->at(i1)[i2]-hped));
						}
					}
					}
...
bool compareByHitTime(const hits &a, const hits &b){return a.t < b.t;}
bool compareByTLTime(const tlist &a, const tlist &b){return a.t < b.t;}

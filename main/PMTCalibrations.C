...
{
	sprintf(hname,"XArapucaXenPMTCalibrations_deneme.root");
...
//	int NRuns=4;
//	int RunList[4]={466,467,472,473};
	int NRuns=24;
        int RunList[24]={408,409,411,420,424,425,428,429,433,434,441,445,451,452,456,458,466,467,472,473,475,476,480,481};
	...
	for(int i1=0;i1<NRuns;i1++)
	{
		...
		{
			if(RunNo>405 && RunNo<426 && i2==3) continue;
	...
	Calibrations[0]->Fit(tflin,"q","q",407.5,485);
	Calibrations[2]->Fit(tflin,"q","q",407.5,485);
	Calibrations[1]->Fit(tflin,"q","q",407.5,425);
// 	Calibrations[1]->Fit(tflin,"q+","q",296,340);
	Calibrations_1->Fit(tflin,"q","q",425,485);
	Calibrations[3]->Fit(tflin,"q","q",425,485);
